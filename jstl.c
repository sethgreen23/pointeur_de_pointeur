...
jstl jstl_create(int len){
  ...
  jstl p = tools_malloc(sizeof(struct _jstl));
  p->len=len;
  p->alloc=len;
  p->data = tools_malloc(sizeof(char)*len);
  int i;
  for(int i=0;i<len;i++){
    p->data[i]=0;
  }
...
jstl empty_jstl_create(int alloc){
  ...
  p->len=0;
  p->alloc=alloc;
  p->data = tools_malloc(sizeof(char)*alloc);
  int i;
...
void jstl_destroy(jstl tab){
  ...
  tools_free(tab,sizeof(struct _jstl));
}
...
jstl jstl_clone(jstl tab){
  jstl clone = standard_empty_jstl_create();
  int i;
  for(i=0;i<tab->len;i++){
    jstl_add(clone,tab->data[i]);
  }
...
regstr jstl_to_regstr(jstl tab){
  regstr word = tools_malloc(sizeof(char)*tab->len+1);
  int i;
  for(i=0;i<tab->len;i++){
    word[i]=tab->data[i];
  }
